...
// const DEFAULT_PLACEHOLDER = `---
// publishDate: 2025-11-27T14:32:22Z
// author: "Mason Everett"
// title: "10 Best Stainless Steel Loaf Pan for Baking in 2025"
// excerpt: "Struggling to bake the perfect bread? We tested the top stainless steel loaf pan to help you find durable, non toxic cookware for your kitchen today."
// image: https://res.cloudinary.com/dpib87gkc/image/upload/v1764167515/blog_image_1764167515_jhbrmm.webp
// category: "Reviews"
// tags:
//   - stainless steel loaf pan
//   - Reviews products
// metadata:
//   canonical: https://www.homeessentialsguide.com/best-stainless- steel-loaf-pan
// ---`;
...
//     // ✅ Generate clean, slug-safe filename from canonical
//     let urlSlug = result.canonicalFound || result.filename;

//     // Replace spaces in URL
//     urlSlug = urlSlug.replace(/\s+/g, '-');

//     // Get last segment after '/'
//     urlSlug = urlSlug.split('/').filter(Boolean).pop() || 'untitled';

//     // Slugify: remove unsafe chars, collapse multiple hyphens, trim edges
//     const cleanFilename = urlSlug
//       .replace(/[^a-zA-Z0-9\-]+/g, '-') // replace unsafe chars with hyphen
...
// Helper to sanitize canonical URL inside MDX
const sanitizeCanonical = (mdxText: string) => {
  return mdxText.replace(
    /^(metadata:\s*canonical:\s*)(.+)$/m,
    (_, prefix, url) => {
      const cleanUrl = url.replace(/\s+/g, ''); // remove spaces inside URL
...
const App: React.FC = () => {
  ...
  const handleLoadSample = () => {
    const cleanContent = sanitizeCanonical(DEFAULT_PLACEHOLDER);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            <div className="relative flex-grow flex flex-col min-h-[500px] h-[calc(100vh-250px)] rounded-xl border border-slate-800 bg-slate-900/50 shadow-xl overflow-hidden group focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all">
              ...
              <textarea
                value={content}
                onChange={(e) => setContent(sanitizeCanonical(e.target.value))}
                placeholder="Paste your MDX content here..."
                className="flex-grow w-full bg-transparent p-4 mt-10 text-sm sm:text-base font-mono leading-relaxed text-slate-300 resize-none focus:outline-none placeholder-slate-700"
                spellCheck={false}
              />
